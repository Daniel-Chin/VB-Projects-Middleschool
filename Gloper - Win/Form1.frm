...
Begin VB.Form Form1 
   ...
   Begin VB.Label Starter 
      ...
      Left            =   120
      TabIndex        =   2
      ToolTipText     =   "开发者:iGlope@163.com"
      Top             =   4320
      Width           =   6972
   End
   Begin VB.Label Starter 
...
Const WinWidth As Long = 10666, WinHeight As Long = 8666
...
If GameOn = True And Holding = True And HuanChong = False Then
    With Player
        ...
        If .Top <= 0 Or Me.Height <= .Top + .Height Then
            Die
        End If
        If .Left <= 0 Or Me.Width <= .Left + .Width Then
            Die
        End If
    End With
    HuanChong = True
...
If Music(-Zz).Controls.currentPosition >= 6.6 Or Music(-Zz).Controls.currentPosition = 0 Then
    With Music(1 + Zz)
        .URL = App.Path & "\music.wma"
        .Controls.currentPosition = 0.1
        .Controls.play
    End With
    Zz = Not Zz
    DJ.Interval = 1000
...
If GameOn Then
    ...
    For k = 0 To 3
        With E(k)
            ...
            If En(k).Top + En(k).Height - HSBC <= 0 Or En(k).Top + HSBC >= Me.Height Then
                .Top_Speed = -.Top_Speed
            End If
            If En(k).Left + En(k).Width - HSBC <= 0 Or En(k).Left + HSBC >= Me.Width Then
            ...
            If .Speed >= 300 Then
                ...
                If FirstP.Value = 1 Then
                    FirstP.Value = 0
                    SaveSetting "iGlope", "Gloper", "FirstPlay", "0"
                    BianSe = False
                    ShowCursor True
                    MsgBox "发现了吗？有敌对方块的颜色变红！", vbInformation + vbOKOnly
                    MsgBox "敌对方块的速度达到一定界限时，颜色会变红。变红的方块对你的威力不增加，但是会持续为你加分。红方块越多，加分越快！", vbInformation + vbOKOnly, "教程"
                    ShowCursor False
                    BianSe = True
                End If
            End If
...
Ex = Ex + 1
Me.Top = Ex ^ 2 * 5 + (Screen.Height - WinHeight) / 2
If WinHeight - 2 * Me.Top <= 0 Then
    Me.Visible = False
    Exiter.Interval = 0
    Exit Sub
End If
Me.BackColor = Me.BackColor + RGB(5, 5, 0)
Me.Height = WinHeight - 2 * Me.Top
SetLayeredWindowAttributes hwnd, 0, 200 - Ex * 6, 2
End Sub

Private Sub FirstP_Click()
SaveSetting "iGlope", "Gloper", "FirstPlay", FirstP.Value
End Sub
...
With Starter(1)
    ...
    .Top = WinHeight / 2
End With
FirstP.Value = GetSetting("iGlope", "Gloper", "FirstPlay", "1")
With Volu
    .Left = WinWidth - .Width
    .Height = WinHeight
End With
Volu.Value = GetSetting("iGlope", "Gloper", "Volume", 0)
End Sub
...
Select Case b
    ...
    Case Is >= 7
        a = "神探詹姆斯！！！"
End Select
Board = Chr(10) & "评分：" & Chr(10) & Left(Str(Tms), IIf(b > 7, b - 5, 2)) & "阶" & a
...
With Screen
    ...
    En(0).Left = jG * zHb
    En(1).Top = jG
    En(1).Left = WinWidth - jG * zHb - En(1).Width
    En(2).Top = WinHeight - jG - En(2).Height
    En(2).Left = jG * zHb
    En(3).Top = WinHeight - jG - En(3).Height
    En(3).Left = WinWidth - jG * zHb - En(3).Width
End With
With Player
...
Exiter.Interval = 30
Quieter.Interval = 80
SaveSetting "iGlope", "Gloper", "Volume", Volu.Value
End Sub
...
Private Sub Volu_KeyUp(KeyCode As Integer, Shift As Integer)
Form_KeyUp KeyCode, Shift
End Sub

Private Sub Volu_Scroll()
Music(1).settings.volume = 100 - Volu.Value * 10
Music(0).settings.volume = 100 - Volu.Value * 10
End Sub

Private Sub Volu_Change()
Volu_Scroll
End Sub
