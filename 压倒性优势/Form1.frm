...
If John >= Jack Then
    John = John + 1
End If
If Man(MaleList(Jack)).Handsome < Man(MaleList(John)).Handsome Then
    Die True, John
Else
    Die True, Jack
End If
...
Private Sub Fuck()
Dim Bob(1 To 10) As Integer, Alice As Integer
    ...
    Select Case Man(Alice).Type
        Case 0
            Love = Man(Alice).LikeHandsome _
                ...
                * (1 - Man(Bob(k)).Handsome)
        Case 1
            Love = Man(Bob(k)).Handsome
        Case 2
            Love = 1 - Man(Bob(k)).Handsome
    End Select
...
With Man(Population)
    ...
    .LikeHandsome = .LikeHandsome + (Violate * 2 * Rnd - Violate)
